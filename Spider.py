...
class Spider(Process):
    ...
    def get_data(self, url,i):
        try:
            requests.get(url, proxies=total_proxies[i])
        except :
        ...
        m = movie()
        context=requests.get(url, proxies=total_proxies[i]).text
        # movie id 从url中就可获得
        ...
        while m.title == []:
            ...
            time.sleep(10)
            i=random.randint(0,len(total_proxies)-1)
            r = requests.get(url, proxies=total_proxies[i])
    ...
    def get_review(self, url,i):  # 爬影评中前200个打分
        # i=random.randint(0,len(total_proxies)-1)

        review_url = url + 'reviews'
        try:
            requests.get(review_url, proxies=total_proxies[i])
        except :
            print("Connection Refused. Try Again.")
            i = random.randint(0, len(total_proxies) - 1)
            requests.get(review_url, proxies=total_proxies[i])
        context = requests.get(review_url,proxies=total_proxies[i]).text
